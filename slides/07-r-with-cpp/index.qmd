...
- [Register as an Apple developer (for free)](https://developer.apple.com/programs/enroll/)
...
```{cpp, eval = FALSE}
// Compute the sum over a numeric vector
...
```{cpp, eval = FALSE}
// Euclidean distances between x and each value in y
NumericVector distC(double x, NumericVector y) {
  int n = y.size();
  NumericVector dist(n);
  for(int i = 0; i < n; ++i) {
    dist[i] = sqrt(pow(x - y[i], 2.0));
  }
  return out;
}
```
...
```{cpp, eval = FALSE}
// Compute sum of each row of a matrix
...
```{.cpp}
Rcpp::rgamma(1, 3, 1 / (y * y + 4))[0]; // n, shape, scale = 1 / rate
```

Or use the `R::rgamma()` function to sample a single value directly

```{.cpp}
R::rgamma(3, 1 / (y * y + 4)); // shape, scale
```
...
gibbs_r <- function(N, thin) {
  ...
  for (i in 1:N) {
    for (j in 1:thin) {
      x <- rgamma(1, 3, y * y + 4) # n, shape, rate 
...
```{cpp, eval = FALSE}
// Compute maximum for each row of a matrix
...
What happens if you use `4` instead of `4.0` here?
...
# Feedback {.inverse .center}

<https://warwick.ac.uk/ellakaye/r-programming-2025-feedback>
